...
public static void bubbleSort(int[] arr) {
    ...
    // i 表示最多需要进行几次冒泡排序，9 个数字最多需要 8 次冒泡排序，剩下的那一个数字自然为最小值:
    for (int i = 0; i < arr.length - 1; i++) {
        // 如果上一次冒泡排序没有发生过交换，表明剩余部分已经有序，排序完成:
        if (!swapped) {
            break;
        }
        swapped = false;
        // j 表示每次进行冒泡排序的数组范围，每经过一次冒泡排序，都有一个最大元素被交换到最后，它不需要再参与到下一次的冒泡排序中:
