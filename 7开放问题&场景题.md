...
不同于 Node.js 和 Deno，Bun 并没有基于 `V8` 引擎，而是**使用了 JavaScriptCore引擎，它的执行速度往往要比 `V8` 等更传统引擎要快**。

**2、语言差异**

`Bun.js`**使用的是新兴的系统编程语言 ZIG编写的**，主要通过手动内存管理对内存进行更细粒度的控制、无隐藏的控制流来提升程序的性能；
...
目前bun还没有开放使用，需要加入他们的discord频道然后发送I want bun才行，最近才收到bun的repository

bun主要是面向服务端，对 windows 用户不太友好，安装起来并不容易，所以只是在Linux 上安装并进行简单尝鲜。
...
bun的配置文件`bunfig.toml`

它允许您在 `bunfig.toml` 加载配置，而不是每次都将参数传递给命令行。

和webpack.config.js类似，也有loaders，external等属性



### 总结

**重点仍然不在生产上。它专注于开发和与现有框架和工具的兼容性。**

**all in one**

快：JavaScriptCore替代V8，使用Zig语言

**我认为的原因**

老实说官网给出的原因我觉得说服力是不够的。依我看来，可能是下面这些原因导致Bun目前性能好于Node

- Bun还不成熟，很多工业级的功能都没有，等加上这些功能后是不是还比Node快真不知道，比如暂时没有Source Maps、Tree-shaking和css压缩器等等
...
页面布局很简单，就是两个 video 标签，分别代表 A 和 B。
...
这部分主要就是分别创建 peer 实例，并互相交换 ICE 信息。
...
后端一个配置https的后端服务；集成websocket，并做基本的消息处理：
