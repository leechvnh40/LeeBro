...
Vue 还提供了 `model` 选项，用于将属性或事件名称改为其他名称，上面的 `MyInput` 组件，改成：
...
`$listeners`：包含父作用域里 **.native 除外**的监听**事件集合**。如果还要继续传给子组件内部的其他组件，就可以通过 v-on="$linteners"
...
- 监听数据必须是**data中声明过或者父组件传递过来的props中的数据**，当数据变化时，触发其他操作，函数有两个参数：
...
2. 在每句css选择器的末尾（编译后的生成的css语句）加一个当前组件的data**属性选择器**（如[data-v-19fca230]）来私有化样式
...
- history
  ...
  - 利用了HTML5 History Interface 中新增的**pushState()和replaceState()方法**。
