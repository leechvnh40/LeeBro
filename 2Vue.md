...
函数式组件就是函数是组件，组件是函数，它的特征**是没有内部状态、没有生命周期钩子函数、没有this**（不需要实例化的组件）。
...
上面已经反复强调，凡是不需要实例化，无状态，没有生命周期的组件，除了`props`之外没有别的配置项，都可以改写成函数式组件。
...
生命周期（父子组件） 父组件beforeCreate --> 父组件created --> 父组件beforeMount --> 子组件beforeCreate --> 子组件created --> 子组件beforeMount --> **子组件 mounted --> 父组件mounted**
...
使用模块（.js或.vue文件）来管理全局变量，最后使用`export`暴露出去 （最好导出的格式为对象，方便在其他地方调用），当其它地方需要使用时，用`import`导入该模块

#### 2.通过全局变量挂载到Vue.prototype

`Global.vue`文件同上，在项目入口的`main.js`里配置：
...
挂载之后，在需要引用全局变量的模块处，不需再导入全局变量模块，而是直接用`this`就可以引用了，如下:
...
使用的时候，在需要引用全局变量的模块处直接使用`this.$store`调用
...
`plugin.js`文件，文件位置可以放在跟`main.js`同一级，方便引用
...
所以要在服务器Ngnix中将所有请求都转发到index.html上就可以了。 
...
vuex相比localstorage或sessionstorage来说，存储数据更安全些。与此同时，vuex也存在一些弊端，当页面刷新后，vuex中state存储的数据同时也会被更新，vuex中存储的数据不能持久化，需要监听处理来维持vuex存储的数据状态持久化。
...
https://juejin.cn/post/6940454764421316644#heading-3x``