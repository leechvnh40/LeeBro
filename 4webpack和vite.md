...
![image.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6ae2804df7064ee183b64c391e229f09~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp) 
...
项目的 package.json 是**配置和描述如何与程序交互和运行的中心**。

npm CLI（和 yarn）用它来**识别你的项目并了解如何处理项目的依赖关系**。

package.json 文件使 npm 可以**启动你的项目、运行脚本、安装依赖项、发布到 NPM 注册表以及许多其他有用的任务**。 

npm CLI 也是管理 package.json 的最佳方法，因为它有助于在项目的整个生命周期内生成和更新 package.json 文件。
...
终端命令，我们把它们放入 `scripts` 字段，可以既可以记录它们又可以轻松地重用。
...
`package.json`记录的是当前项目中你下载了哪些包（也即`npm install xx` 的包信息），记录了你下载的包信息（地址、版本号等），不包含依赖包信息。
...
**package-lock.json是在npm v5中新增的，npm v5的依赖树改为扁平化，同时利用lock记录package.json的依赖**

### npm ci和npm i

##### npm ci 命令

是通过加载项目中 package-lock.json或npm-shrinkwrap.json构建node_modules树。

##### npm i （npm install）命令

是通过加载项目中 package.json构建node_modules树。

##### npm ci与npm i主要有以下的区别。(npm 版本要 ≥ v5.7.1)
